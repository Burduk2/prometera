...
{#if $activeModal === name}
  ...
  on:click={e => e.target === wrapper ? toggleModal(name, false) : 0} bind:this={wrapper}
  transition:fade={{duration: 150}}>
    <div class="relative bg-bg-main p-10 rounded-lg max-w-full max-h-[90%] overflow-y-scroll" style="width: {w}px"
    transition:fly={{y: 50, duration: 200}}>
      <button on:click={() => toggleModal(name, false)} title="Close" class="absolute top-4 right-4 cursor-pointer hover:opacity-80">
